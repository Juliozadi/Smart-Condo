...
const api = {
  paragrafos: [
    'A comunicação entre o front-end e o banco de dados acontece por uma API REST, com 98 endpoints distribuídos em treze módulos. Todos os endereços começam com /api/v1, o que permite publicar uma versão 2 no futuro sem quebrar as telas que já usam a versão atual.',
...
const testes = {
  paragrafos: [
    'As regras do sistema são verificadas por uma suíte de 298 casos de teste automatizados, escritos com pytest e executados contra um banco PostgreSQL real, e não contra um banco simulado. Assim, restrições de chave estrangeira e de unicidade também são exercitadas.',
    ...
    'Além da suíte do servidor, 55 testes de interface abrem as telas em um navegador Chromium real, nos temas claro e escuro, em largura de computador e de celular. Eles conferem o que já falhou uma vez e não pode voltar: erros de JavaScript, imagens quebradas, rolagem lateral no celular, a barra superior fora do topo, títulos e cartões sem ícone, caixas claras no tema escuro, elementos marcados como escondidos que continuavam aparecendo, a logo levando ao painel de cada perfil e o medidor de força da senha. Três deles percorrem os fluxos com arquivo de ponta a ponta: a captura pela câmera, com uma câmera simulada do navegador, a foto tirada pelo porteiro chegando ao painel do morador, e os documentos escolhidos no cadastro chegando à fila de aprovação do síndico. Também rodam a cada envio, com o banco, a API e o site no ar.',
...
const conclusao = [
  ...
  'Até o momento, o sistema conta com as quatro áreas de acesso implementadas e ligadas à API, 21 tabelas em PostgreSQL, 98 endpoints, 298 casos de teste automatizados cobrindo as regras de negócio e 55 testes de interface executados em um navegador real.',
