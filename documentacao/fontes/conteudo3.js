...
const acessibilidade = {
  ...
  itens: [
    { n: '12.1', nome: 'Boas práticas', paragrafos: [
      ...
      'Além do contraste, as telas passam por uma auditoria automática com o axe-core, a ferramenta de referência para as regras da WCAG. A primeira execução encontrou problemas que não se veem na tela: o botão de acessibilidade não recebia o foco do teclado — quem não usa mouse não conseguia abrir justamente as ferramentas de acessibilidade —, a tabela de visitantes era anunciada sem as células, os links de voltar e de perfil ficavam sem texto no celular, e vinte páginas não tinham um título principal. Corrigidos, a auditoria roda a cada envio de código em todas as telas, nos dois tamanhos de tela, e acusa qualquer regressão.',
    ]},
...
const arquitetura = {
  ...
  subsecoes: [
    ...
    { n: '13.2', nome: 'Segurança dos dados', paragrafos: [
      ...
      'Para que a senha não possa ser descoberta por tentativa e erro, o login conta as tentativas malsucedidas e bloqueia a conta por quinze minutos depois de cinco senhas erradas seguidas. O bloqueio é temporário de propósito: fosse permanente, bastaria errar a senha de alguém repetidamente para deixá-lo fora do sistema. Acertar a senha zera a contagem. A verificação do bloqueio acontece antes da conferência da senha, de modo que uma conta bloqueada não consuma processamento a cada nova tentativa. As tentativas da mesma conta são conferidas uma por vez: se fossem todas ao mesmo tempo, cada uma leria a contagem antes de as outras a aumentarem, e o bloqueio não chegaria a disparar.',
      ...
      'As regras que dependem da data — reserva em horário que já passou, cobrança vencida, data de nascimento no futuro — usam o dia e a hora do fuso do condomínio, configurado na API, e não o relógio da máquina. O servidor que hospeda a aplicação costuma estar no horário universal, quatro horas à frente de Campo Grande; sem isso, às seis da tarde uma reserva para as sete da noite seria recusada como horário que já passou.',
...
const testes = {
  paragrafos: [
    'As regras do sistema são verificadas por uma suíte de 353 casos de teste automatizados, escritos com pytest e executados contra um banco PostgreSQL real, e não contra um banco simulado. Assim, restrições de chave estrangeira e de unicidade também são exercitadas.',
  ...
  tabela: [
    ...
    ['test_auth.py', '42', 'Cadastro, código de confirmação, login, bloqueio por tentativas, recuperação de senha e sessões encerradas pela troca de senha'],
...
const conclusao = [
  ...
  'Até o momento, o sistema conta com as quatro áreas de acesso implementadas e ligadas à API, 21 tabelas em PostgreSQL, 99 endpoints, 353 casos de teste automatizados cobrindo as regras de negócio e 75 testes de interface executados em um navegador real.',
